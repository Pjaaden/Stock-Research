...
- `结构化数据层`：`data/*.json`（工作台消费字段） + `<research_folder>/data/quarterly/*.csv`（季度财报会计时间序列；规范见 `TRAE/QUARTERLY_CSV_WORKFLOW.md`）
...
- 凡涉及财报（季报/半年报/年报）的任务，必须按 `TRAE/QUARTERLY_CSV_WORKFLOW.md` 同步 `<research_folder>/data/quarterly/` 的季度 CSV 归档——该规范适用于**所有研究标的**（未建档标的在本次触及该标的时补齐）

## 默认工作顺序（每次任务都遵守）

1. 判断任务属于：新立项 / 财报更新 / 事件跟踪 / JSON 同步 / 候选池维护 / 季度 CSV 归档
2. 读取该标的已有目录、最近两份报告、`README.md`（如存在）以及相关 `data/*.json` 与 `data/quarterly/`（如存在）
3. 明确哪些是新事实，哪些是旧观点修正（不要把“新消息”直接写成“新结论”）
4. 先产出研究文件（报告/附录/事件复盘）
5. 同步季度 CSV：按 `TRAE/QUARTERLY_CSV_WORKFLOW.md` 追加/补齐 `<research_folder>/data/quarterly/`（财报类任务强制）
6. 再给出结构化更新建议（`data/*.json` 字段级）
7. 如需同步仓库：明确目标路径、文件名、README 更新点和 JSON 字段变更
...
- 不要只出研究 HTML 而跳过季度 CSV 归档（财报类任务必须留时序底稿）
...
- 季度 CSV 归档规范（全标的适用）：`TRAE/QUARTERLY_CSV_WORKFLOW.md`
- 试运行报告：`TRAE/reports/`
