...
- **进行中**：凌霄泵业 2026Q3 CSV 追加（10 月末）｜ 其余 7 标的季度 CSV 建档（清单见规范 §6.1）｜ 已投资标的研究补全（霸王茶姬、凌霄泵业）
- **最近动态**（新→旧）：2026-09-10 赛轮集团 2026H1 中报深度解析 v21 + 季度财报 CSV 建档 + **CSV 归档升级为全标的项目标准**（`WORKFLOW.md` 新增第4步强制、`PROJECT_INSTRUCTIONS.md` 三层结构/输出规则同步；8节量化报告，情景加权中枢16.00元；`data/quarterly/` 4份CSV，16项恒等式校验通过；companies.json 赛轮字段写回）→ 2026-09-02 霸王茶姬 2026Q2 v7（读数框架/直营核查/组织验证/SOTP估值，companies.json 写回）→ 2026-08-27 工作流优化（sessions/ 交接档案 + HANDOFF + 多设备规范）→ 2026-08-27 凌霄泵业 2026H1 报告 + 季度 CSV 建档 → 2026-08-26 本地化迁移与连通性测试
...
| 赛轮集团 | 601058.SH | active | hold | `赛轮集团_601058` | 35 |
...
> 季度 CSV 建档：✅ 凌霄泵业、赛轮集团｜⬜ 其余 7 家待建档（覆盖清单见 `TRAE/QUARTERLY_CSV_WORKFLOW.md` §6.1）
...
- `QUARTERLY_CSV_WORKFLOW.md`：**季度财报 CSV 存档规范（项目标准 · 全标的适用）**——凡财报类任务必须按规范同步 `<research_folder>/data/quarterly/`；已建档：凌霄泵业 002884、赛轮集团 601058（覆盖清单见规范 §6.1）
